...
def runNonFed(data_info, method = 'FairBatch', learning_rate = 0.005, num_epochs = 300, alpha = 1, num_features = 3,
                optimizer = 'adam', seed = 123, penalty = 500, Z = 2, lr_g = 0.005, lr_d = 0.01, 
                  ratio_gd = 3,
    ...
    for c in range(len(clients_idx)):
        ...
        if method == 'FairBatch':
            ...
            acc_, fair_ = server.FairConstraints(num_epochs = num_epochs, learning_rate = learning_rate, penalty = penalty,
                    optimizer = optimizer)
        elif method == 'FTrain':
            acc_, fair_ = server.FTrain(num_epochs = num_epochs, lr_g = lr_g, lr_d = lr_d, num_classes = Z, ratio_gd = ratio_gd, lambda_d = lambda_d, init_epochs = init_epochs)
        n += len(clients_idx[c])
