...
def loss_with_fair(output, target, Theta_X, Sen, Sen_bar):
    ...
    #fair_loss = torch.mean(torch.abs(fair_loss))
...
for i in range(Iteration):
    theta, bias = compute_model_parameter_fairness(weight, Sen_torch, ratio_fair, options[2])
    ...
    #print(weight[0:10])
    weight, ratio = compute_alpha_with_fairness(theta, bias, Xtr, Ytr, Sen, Xref, sigma, B, tau, k)
