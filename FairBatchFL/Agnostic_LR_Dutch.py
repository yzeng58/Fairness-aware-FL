...
num_epochs = 20
filename = '/home/wd005/dutch_norm.csv'
numpy_data = genfromtxt(filename, delimiter=',')
numpy_data = numpy_data[np.argsort(numpy_data[:,6])]

count = 0

train = []
split_ratio = 0.2
male = int(batch_size*split_ratio*2)
...
index = batch_size
train_data = numpy_data[:, 1:-1][0:index-10000]
train_label = numpy_data[:, -1][0:index-10000]
...
def loss_with_fair(output, target, Theta_X, Sen, Sen_bar):
    ...
    return pred_loss + fair_factor*fair_loss
...
def loss_with_local_fair(output, target, Theta_X, Sen, k):
    ...
    print(pred_loss)
    print(fair_loss)
    return pred_loss + 3.0*fair_loss
...
def compute_model_parameter_fairness(weight, Sen, ratio, option, k):
    ...
    for epoch in range(num_epochs):
        ...
        Sen_train = numpy_data[:, 0][0:index-10000].reshape(-1, 1)
        ...
        # for i in range(k):
        #     if i == 0:
        #         Sen_train_j = Sen_test[0: 2000]
        #         pred_j = pred[0: 2000]
        #         label_j = label[0:2000]
        #     else:
        #         Sen_train_j = Sen_test[2000:4000]
        #         pred_j = pred[2000:4000]
        #         label_j = label[2000:4000]
        #     risk_difference = risk_difference_calculation(Sen_train_j, pred_j)
        #     accuracy = torch.sum((pred_j[0:4000] == label_j[0:4000])).numpy()
        ...
        risk_difference = risk_difference_calculation(Sen_test, pred)
...
Sen_torch = torch.from_numpy(Sen).float()[0:index]
...
for i in range(Iteration):
    ...
    theta, bias = compute_model_parameter_fairness(weight, Sen_torch, ratio, options[0], k)
